...
export function LogoMark({ className = "h-6 w-auto", tone = "ink" }: LogoProps) {
  return (
    <Image
      src={markSrc[tone]}
      alt=""
...
export default function Logo({ className = "", tone = "ink" }: LogoProps) {
  const textColor = tone === "white" ? "text-white" : "text-ink";
  return (
    <span className={`inline-flex items-center gap-2.5 ${className}`}>
      <LogoMark className="h-6 w-auto sm:h-7" tone={tone} />
      <span
